  ...
  class DashboardsController < AdminController
    ...
    def growth
      sales_growth_of_past_week = Order
                                  ...
                                  .sum(:total_amount)
      sales_growth_of_current_week =
        Order.where('created_at >= ?', 1.week.ago).sum(:total_amount)
      if sales_growth_of_past_week != 0
        growth = (((
          sales_growth_of_current_week - sales_growth_of_past_week
        ) / sales_growth_of_past_week
                  ) * 100).to_i
      else
        sales_growth_of_current_week
      end
